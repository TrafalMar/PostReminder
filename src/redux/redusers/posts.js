...
const reduser = (state = initialState, action) => {
  ...
  const addPost = (userId) => {
    ...
    return {
      ...state,
      posts: { ...newPosts, [action.time + userId]: newPost },
    };
  };
  ...
  const addField = (postId, fieldType) => {
    ...
    if (
      newPosts[postId].items === undefined ||
      newPosts[postId].items === null
    ) {
      newPosts[postId] = {
        ...newPosts[postId],
        items: [],
  ...
  const deleteField = (postId, itemId) => {
    // delete here
    newPosts[postId].items = Object.keys(newPosts[postId].items)
      .filter((key) => key !== itemId)
      .map((key) => newPosts[postId].items[key]);
  ...
  const deletePost = (postId) => {
    ...
    const updatedPosts = Object.assign(
      {},
      ...Array.from(filteredKeys, (key) => ({ [key]: newPosts[key] }))
    );
  ...
  const closeSettings = () => ({ ...state, showSettings: false });

  const togglePostPrivacy = () => {
    newPosts[state.focusedPostId].settings.private = !newPosts[
      state.focusedPostId
    ].settings.private;
    return { ...state, posts: { ...newPosts } };
  };
  ...
  const dragHappened = (action) => {
    ...
    if (droppableIdStart === droppableIdEnd) {
      const item = newPosts[droppableIdStart].items.splice(
        droppableIndexStart,
        1
      );
