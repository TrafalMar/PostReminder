      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <Aux>
          ...
          {this.props.isAuthenticated ? (
            <PostsControls
              addPost={() => this.props.addPost(this.props.userId)}
            />
          ) : null}
...
const mapDispatchToProps = (dispatch) => ({
  ...
  toggleEditMode: (editMode, key, token) =>
    dispatch(action.toggleEditMode(editMode, key, token)),
  ...
  changeField: (postId, itemId, input) =>
    dispatch(action.changeField(postId, itemId, input)),
  deletePost: (postId) => dispatch(action.deletePost(postId)),
  initPosts: (token, userId) => dispatch(action.initPosts(token, userId)),
  sort: (sId, eId, sIndex, eIndex, dId) =>
    dispatch(action.sort(sId, eId, sIndex, eIndex, dId)),
});
