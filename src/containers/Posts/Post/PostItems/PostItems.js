...
const PostItems = (props) => {
    ...
    Object.keys(props.items).map((key, index) => {
      const deleteButton = (
        <CustomButton
          ...
          onClick={() => props.deleteField(props.postId, key)}
        >
          <FaTimes />
        </CustomButton>
      );
      ...
      switch (props.items[key].type) {
        case "Title":
          formItems.push(
            <p key={key} className={classes.Header}>
              {props.items[key].context}
            </p>
          );
          editFormItems.push(
            <Draggable
              draggableId={String(props.items[key].id)}
              index={index}
                  ...
                  <div className={classes.PostItem}>
                    ...
                    <div className={classes.EditLine}>
                      ...
                      <input
                        key={key}
                        onChange={(event) => {
                          props.changeField(props.postId, key, event);
                        }}
        ...
        case "Paragraph":
          formItems.push(
            <p key={key} className={classes.Details}>
              {props.items[key].context}
            </p>
          );
          editFormItems.push(
  ...
  const content =
    props.items !== undefined ? (
      ...
      <p style={{ textAlign: "center" }}>You are welcome to add new items</p>
    );
